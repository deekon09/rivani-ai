...
function lukiFallbackReply(raw){
  ...
  if(/(plan|free|pro|premium|price|billing|limit|9 per day|9\/day)/.test(q)){const c=window.RIVANI_LUKI_CONTEXT||{};const prefix=c.signedIn?`You’re currently on the ${c.plan||'Free'} plan${c.username?`, ${c.username}`:''}. `:'';return prefix+'The three live tools each use a 9-successful-jobs-per-day Free limit. Background Remover keeps full Precision quality on Free; Image Enhancer keeps its standard quality controls on Free; Audio Repair includes Clear Voice and MP3 on Free. Pro benefits vary by tool, but public billing is not connected yet, so any displayed price is not a completed checkout.';}
  if(/(all tool|which tool|tools|feature|about|rivani|platform|live)/.test(q))return 'RIVANI AI currently has three live focused tools: AI Audio Repair, Image Enhancer and Background Remover. Each has its own page and detailed guide, browser-first processing where implemented, a 9-successful-jobs-per-day Free allowance, and tool-specific Pro benefits. LUKI can explain any control, limit, export or workflow for these three tools.';
  ...
  return 'I can help with the three live RIVANI AI tools — Audio Repair, Image Enhancer and Background Remover — plus accounts, Free/Pro limits, exports, policies and website navigation.';
}
