...
queueMicrotask(()=>{if(el.engineNote)el.engineNote.textContent="V45.3 ENGINE ACTIVE · human-only polish loaded";console.info("RIVANI Video Studio",RIVANI_VIDEO_BUILD);});
...
el.clear?.addEventListener("click",()=>{
  ...
  el.preview.disabled=true;el.exportBtn.disabled=true;
  el.engineStatus.textContent="Not loaded";el.personStatus.textContent="Waiting";el.faceStatus.textContent="Waiting";el.stabilityStatus.textContent="Waiting";
  el.engineNote.textContent="V45.3 ENGINE ACTIVE · human-only polish loaded";
});
  ...
  for(let i=0,j=0;i<n;i++,j+=4){
    ...
    if(classes){
      ...
      if(cls===0){
        ...
        target=target>.92?target*.38:target*.06;
      }
    }
