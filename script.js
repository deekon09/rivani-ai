// RIVANI AI V37 — Beta access + brand SEO + interaction-safe slider + LUKI + Image Compressor + Image to Text OCR
...
if(!document.getElementById('lukiLauncher')){document.body.insertAdjacentHTML('beforeend',`<button class="luki-launcher" id="lukiLauncher" type="button" aria-label="Open LUKI assistant" aria-expanded="false"><span class="luki-launcher-orb rivani-r-crop luki-r-logo" aria-hidden="true"><img src="assets/rivani-ai-logo.png" alt="" decoding="async"></span><span class="luki-launcher-copy"><strong>LUKI</strong><small>Ask about RIVANI AI</small></span><span class="luki-online-dot"></span></button><aside class="luki-panel" id="lukiPanel" aria-label="LUKI RIVANI assistant" aria-hidden="true"><div class="luki-head"><div class="luki-avatar rivani-r-crop luki-r-logo" aria-hidden="true"><img src="assets/rivani-ai-logo.png" alt="" decoding="async"></div><div><strong>LUKI</strong><span>RIVANI AI Assistant</span></div><button class="luki-close" id="lukiClose" type="button" aria-label="Close LUKI">×</button></div><div class="luki-scope"><span>✦</span>Ask naturally. LUKI understands RIVANI tools, Beta access, accounts, policies and roadmap.</div><div class="luki-messages" id="lukiMessages" aria-live="polite"><div class="luki-message bot"><div class="luki-bubble">Hi, I’m LUKI. Five RIVANI tools are live in Public Beta All Access right now. Ask me about Audio Repair, Image Enhancer, Background Remover, Smart Image Compressor, Image to Text / OCR, accounts or the roadmap.</div></div></div><div class="luki-quick" id="lukiQuick"><button type="button" data-question="What can AI Audio Repair do?">Audio Repair</button><button type="button" data-question="What can Image to Text OCR do?">Image to Text</button><button type="button" data-question="What can Smart Image Compressor do?">Compressor</button><button type="button" data-question="Which RIVANI tools are live in Beta?">Beta tools</button><button type="button" data-question="What does Beta All Access include?">All Access</button><button type="button" data-question="Which RIVANI tools are upcoming?">Upcoming</button></div><form class="luki-form" id="lukiForm"><input id="lukiInput" type="text" maxlength="500" autocomplete="off" placeholder="Ask LUKI about RIVANI AI…"><button type="submit" aria-label="Send question">➜</button></form><div class="luki-foot">RIVANI help only · AI responses can be imperfect</div></aside>`)}
...
function lukiFallbackReply(raw){
  ...
  if(/(image to text|ocr|extract text|screenshot to text|photo to text|scan to text|receipt|table to csv|multilingual ocr|handwriting)/.test(q))return 'Image to Text / OCR is live in Public Beta All Access. It extracts editable text from photos, screenshots, scanned pages and receipts in the browser, supports Indian languages plus major world-language OCR choices including Spanish, French, German, Portuguese, Arabic, Chinese, Japanese, Korean and Russian, and includes Auto Clean, Tiny Text Upscale, Smart OCR Race, confidence review, batch extraction, privacy-pattern scan and TXT/Markdown/JSON plus best-effort table-to-CSV export. Handwriting mode is experimental and important names, numbers and official text should always be reviewed.';
  if(/(compressor|compress image|image compress|exact kb|20 ?kb|50 ?kb|100 ?kb|200 ?kb|format race|quality guard|website pack|reduce image size)/.test(q))return 'Smart Image Compressor is live in Public Beta All Access. It can reduce image file size or aim for exact KB targets, with Best Quality + Smaller Size as the recommended default, Visual Quality Guard, Text & Logo Guard, Transparency Guard, Smart Format Race, batch consistency and Website Pack output. Compression is browser-side and sign-in is required before processing.';
  ...
  if(/(calculator|scientific|fraction|percentage|equation|quadratic|statistics|geometry|converter|emi|interest|student math)/.test(q))return 'The Advanced Student Calculator is marked Upcoming while RIVANI expands its five live public-Beta tools. It is not being promoted as a live tool right now.';
  if(/(upcoming|roadmap|passport|resizer|compressor|converter|ocr|object remover|pdf|subtitle|transcription|rewriter|grammar|qr|photo restorer|colorizer)/.test(q))return 'RIVANI’s roadmap includes PDF Toolkit, AI Resume Builder, AI Logo Generator, Advanced Student Calculator, Passport Photo Maker, Object Remover, photo restoration/colorization, subtitle/transcription tools, text utilities and QR tools. Image to Text / OCR and Smart Image Compressor are already live. Upcoming means planned, not live.';
  ...
  if(/(plan|free|pro|premium|price|billing|payment|upi|qr|limit|daily|unlimited|all access)/.test(q)){const c=window.RIVANI_LUKI_CONTEXT||{};const prefix=c.signedIn?`You’re signed in${c.username?` as ${c.username}`:''}. `:'';return prefix+'RIVANI is currently running Public Beta All Access for AI Audio Repair, Image Enhancer, Background Remover, Smart Image Compressor and Image to Text / OCR: current implemented controls are temporarily unlocked and there is no successful-job daily cap. Future paid Pro/Premium tiers are not currently for sale; no public price or QR/UPI checkout is active.';}
  if(/(all tool|which tool|tools|feature|about|rivani|platform|live|beta)/.test(q))return 'RIVANI AI currently has five live public-Beta tools: AI Audio Repair, Image Enhancer, Background Remover, Smart Image Compressor and Image to Text / OCR. They are on Beta All Access with current implemented controls unlocked and no successful-job daily cap. PDF Toolkit, AI Resume Builder, AI Logo Generator, Advanced Student Calculator and other roadmap utilities remain Upcoming.';
  ...
  return 'I can help with RIVANI’s five live Beta tools, Beta All Access, upcoming tools, accounts, exports, policies and website navigation.';
}
...
function betaLocalQuestion(q){return /(audio|image|photo|background|remove bg|compressor|compress image|exact kb|image to text|ocr|extract text|screenshot to text|receipt|table to csv|plan|free|pro|premium|price|billing|payment|upi|qr|limit|daily|all access|calculator|upcoming|roadmap|which rivani tools|all tool|tools?|tools live|tool status|contact|turnstile|security check)/i.test(q)}
function staleCommerceAnswer(text){return /(₹\s*(?:199|499)|launch offer|15-minute|15 minute|upi\/qr|manual (?:upi|payment) verification|payment screenshot|calculator is (?:a )?live|live free advanced|9\s*(?:successful|free|\/day|per day)|daily limit|controls? (?:remain|are) locked|planned for rivani pro|image enhancer\s+(?:is\s+)?planned|background remover\s+(?:is\s+)?planned|image compressor\s+(?:is\s+)?planned|image to text\s+(?:is\s+)?planned|ocr\s+(?:is\s+)?planned|three live|three active|four live|four active|flagship,? in development)/i.test(text)}
...
(function installA11yMinimumTargets(){
  ...
  style.textContent='.luki-close,.luki-form button,.luki-quick button,.home-live-controls button,.mobile-menu-btn{min-width:44px;min-height:44px}@media(max-width:560px){.luki-quick{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:7px!important}.luki-quick button{min-width:0!important;width:100%!important;max-width:100%!important;white-space:normal!important;overflow-wrap:anywhere!important;word-break:normal!important;font-size:11px!important;line-height:1.2!important;padding:8px 7px!important;border-radius:14px!important}.luki-panel{max-width:calc(100vw - 16px)!important}}';
