...
const state = {
  ...
  exportBlobUrl: "", recordingMime: "", renderBusy: false, matteRaw: null, matteCore: null, matteNear: null, matteTmp: null, matteRefined: null
};
...
const RIVANI_VIDEO_BUILD="V45.7-HD-PRESERVE";
queueMicrotask(()=>{if(el.engineNote)el.engineNote.textContent="V45.7 ENGINE ACTIVE · connected matte + HD source-quality export loaded";console.info("RIVANI Video Studio",RIVANI_VIDEO_BUILD);});
...
el.clear?.addEventListener("click",()=>{
  ...
  el.preview.disabled=true;el.exportBtn.disabled=true;
  el.engineStatus.textContent="Not loaded";el.personStatus.textContent="Waiting";el.faceStatus.textContent="Waiting";el.stabilityStatus.textContent="Waiting";
  el.engineNote.textContent="V45.7 ENGINE ACTIVE · connected matte + HD source-quality export loaded";
});
...
async function exportTransparentVideo(){
  ...
  // Respect the user's selected/original resolution. Do not silently
  // downgrade a portrait Full-HD source to 608x1080 or force mobile to 720p.
...
function sourceFpsHint(){
  let stream=null;
  try{
    if(typeof el.video.captureStream==="function"){
      stream=el.video.captureStream();
      const track=stream.getVideoTracks?.()[0];
      const fps=Number(track?.getSettings?.().frameRate);
      if(Number.isFinite(fps)&&fps>=15&&fps<=120){
        stream.getTracks().forEach(t=>t.stop());
        const common=[24,25,30,48,50,60];
        return common.reduce((best,v)=>Math.abs(v-fps)<Math.abs(best-fps)?v:best,30);
      }
    }
  }catch(_e){}
