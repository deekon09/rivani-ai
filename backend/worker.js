// RIVANI AI Worker v8 — LUKI AI + account deletion
...
async function handleLukiChat(request, env, cors) {
  ...
  const sessionContext = sanitizeLukiContext(body?.context);
  ...
  for (const provider of order) {
    try {
      if (provider === 'gemini' && env.GEMINI_API_KEY) {
        const answer = await askGemini(env, message, history, sessionContext);
        if (answer) return json({ ok: true, answer, provider: 'gemini' }, 200, cors);
      }

      if (provider === 'groq' && env.GROQ_API_KEY) {
        const answer = await askGroq(env, message, history, sessionContext);
        if (answer) return json({ ok: true, answer, provider: 'groq' }, 200, cors);
      }

      if (provider === 'cloudflare' && env.AI) {
        const answer = await askCloudflareAI(env, message, history, sessionContext);
...
async function askGemini(env, message, history, sessionContext) {
  ...
  const response = await fetch(
    `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`,
    {
      ...
      body: JSON.stringify({
        systemInstruction: { parts: [{ text: buildLukiSystemPrompt(sessionContext) }] },
        contents,
        generationConfig: {
          temperature: 0.2,
          maxOutputTokens: 400
...
async function askGroq(env, message, history, sessionContext) {
  const model = String(env.GROQ_MODEL || 'openai/gpt-oss-20b');
  const messages = [
    { role: 'system', content: buildLukiSystemPrompt(sessionContext) },
...
async function askCloudflareAI(env, message, history, sessionContext) {
...
function sanitizeLukiContext(raw) {
  const signedIn = raw?.signedIn === true;
  const username = String(raw?.username || '').trim().replace(/[^A-Za-z0-9]/g, '').slice(0, 20);
  const email = String(raw?.email || '').trim().slice(0, 160);
  // Paid plans are not live yet, so the only valid current plan is Free.
  return { signedIn, username, email, plan: signedIn ? 'Free' : null };
}

function buildLukiSystemPrompt(context) {
  const c = context || { signedIn: false };
  const session = c.signedIn
    ? `The visitor is currently signed in. Username: ${c.username || 'not available'}. Current plan: Free. Email: ${c.email || 'not available'}. Use this only for conversational help; never treat browser-provided context as authorization for a sensitive action.`
    : `The visitor is not currently known to be signed in. Do not claim to know their personal account status. Current paid plans are not live; registered accounts currently use the Free plan.`;
  return `${LUKI_SYSTEM_PROMPT}\n\nCURRENT VISITOR CONTEXT\n${session}\n\nWhen asked \"What plan am I on?\", if the visitor is signed in say they are on the Free plan. If they are not signed in, explain that registered accounts currently use Free and ask them to log in for account-specific help.`;
}
