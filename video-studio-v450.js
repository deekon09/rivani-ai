...
const RIVANI_VIDEO_BUILD="V45.0-CACHE-RESET";
queueMicrotask(()=>{
  if(el.engineNote) el.engineNote.textContent="V45.0 JS ACTIVE · new engine file loaded · upload a video to begin.";
  console.info("RIVANI Video Studio",RIVANI_VIDEO_BUILD,"person confidence channel = 1");
});
...
async function initSegmenter(){
  ...
  el.engineNote.textContent="V45.0 · Person channel locked · background mask refreshes continuously while the subject moves.";
...
async function segmentCurrent(force=false){
  ...
  // MediaPipe ImageSegmenter SelfieSegmenter contract:
  // confidenceMasks[0] = background, confidenceMasks[1] = person.
  // V45 intentionally does not use category masks or polarity guessing.
  const bgMask=confidence[0]||null;
  const personMask=confidence[1]||null;
  if(confidence.length<2||!personMask){
    throw new Error(`Unexpected SelfieSegmenter output (${confidence.length} mask channel${confidence.length===1?"":"s"}). RIVANI stopped instead of guessing foreground polarity.`);
  }
  const referenceMask=personMask;
  ...
  const personData=personMask.getAsFloat32Array();
  const n=mw*mh;
  if(!personData||personData.length<n){
    throw new Error("Person confidence mask geometry mismatch.");
  }
  ...
  for(let i=0,j=0;i<n;i++,j+=4){
    const target=clamp(Number(personData[i])||0,0,1);
...
function resetControls(){
  el.edgeClean.value=42;el.feather.value=1;el.temporal.value=32;el.light.value=18;el.smooth.value=18;el.glow.value=12;el.clarity.value=10;el.warmth.value=6;
