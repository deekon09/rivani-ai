...
const ALPHA_FFMPEG_WORKER_URL="https://cdn.jsdelivr.net/npm/ffmpeg.js@4.2.9003/ffmpeg-worker-webm.js";
...
const state = {
  ...
  exportBlobUrl: "", recordingMime: "", renderBusy: false, alphaWorkerBlobUrl: ""
};
...
const RIVANI_VIDEO_BUILD="V45.2-ROLLBACK-STABLE";
queueMicrotask(()=>{if(el.engineNote)el.engineNote.textContent="V45.4 ENGINE ACTIVE · human-only + true-alpha export loaded";console.info("RIVANI Video Studio",RIVANI_VIDEO_BUILD);});
...
el.clear?.addEventListener("click",()=>{
  ...
  el.preview.disabled=true;el.exportBtn.disabled=true;
  el.engineStatus.textContent="Not loaded";el.personStatus.textContent="Waiting";el.faceStatus.textContent="Waiting";el.stabilityStatus.textContent="Waiting";
  el.engineNote.textContent="V45.4 ENGINE ACTIVE · human-only + true-alpha export loaded";
});
...
function canvasToMatte(maskCanvas,w,h,matteCanvas){
  if(matteCanvas.width!==w)matteCanvas.width=w;
  if(matteCanvas.height!==h)matteCanvas.height=h;
...
async function getAlphaFfmpegWorker(){
  // ffmpeg.js WebM worker is fetched only when a true-alpha export is requested.
  // It is converted to a same-origin blob worker; source video never leaves device.
  if(!state.alphaWorkerBlobUrl){
    const response=await fetch(ALPHA_FFMPEG_WORKER_URL,{mode:"cors",cache:"force-cache"});
    if(!response.ok)throw new Error(`Alpha encoder download failed (${response.status}).`);
    const code=await response.text();
    state.alphaWorkerBlobUrl=URL.createObjectURL(new Blob([code],{type:"application/javascript"}));
  }
  const worker=new Worker(state.alphaWorkerBlobUrl);
  await new Promise((resolve,reject)=>{
    const timeout=setTimeout(()=>{worker.terminate();reject(new Error("Alpha encoder did not initialize."));},30000);
    const onMessage=e=>{
      const m=e.data||{};
      if(m.type==="ready"){
        clearTimeout(timeout);worker.removeEventListener("message",onMessage);resolve();
      }else if(m.type==="error"||m.type==="abort"){
        clearTimeout(timeout);worker.removeEventListener("message",onMessage);worker.terminate();
        reject(new Error(String(m.data||"Alpha encoder initialization failed.")));
      }
    };
    worker.addEventListener("message",onMessage);
    worker.addEventListener("error",e=>{clearTimeout(timeout);reject(e.error||new Error("Alpha encoder worker failed."));},{once:true});
  });
  return worker;
}
async function mergeAlphaWebM(colorBlob,matteBlob){
  const worker=await getAlphaFfmpegWorker();
  const color=await blobToU8(colorBlob),matte=await blobToU8(matteBlob);
  return await new Promise((resolve,reject)=>{
    let stderr="",settled=false;
    const finishError=message=>{
      if(settled)return;settled=true;worker.terminate();reject(new Error(message));
    };
    const onMessage=e=>{
      const m=e.data||{};
      if(m.type==="stderr"){
        stderr+=String(m.data||"")+"\\n";
        const hit=String(m.data||"").match(/time=(\\d+):(\\d+):(\\d+(?:\\.\\d+)?)/);
        if(hit&&Number.isFinite(el.video.duration)&&el.video.duration>0){
          const sec=Number(hit[1])*3600+Number(hit[2])*60+Number(hit[3]);
          setProgress(70+Math.min(28,28*sec/el.video.duration),"Building transparent WebM…","Encoding VP8 alpha locally · no gradient fallback.");
        }
      }else if(m.type==="done"){
        if(settled)return;
        const files=m.data?.MEMFS||[];
        const file=files.find(f=>f.name==="rivani-transparent.webm")||files[0];
        if(!file?.data?.length)return finishError(`Alpha encoder returned no file. ${stderr.slice(-500)}`);
        settled=true;worker.terminate();
        resolve(new Blob([file.data],{type:"video/webm"}));
      }else if(m.type==="error"||m.type==="abort"){
        finishError(`${String(m.data||"Alpha encoding failed.")} ${stderr.slice(-500)}`.trim());
      }
    };
    worker.addEventListener("message",onMessage);
    worker.addEventListener("error",e=>finishError(e.message||"Alpha encoder worker crashed."),{once:true});
    worker.postMessage({
      type:"run",
      MEMFS:[
        {name:"color.webm",data:color},
        {name:"matte.webm",data:matte}
      ],
      arguments:[
        "-i","color.webm",
        "-i","matte.webm",
...
async function exportTransparentVideo(){
  if(IS_IOS||IS_SAFARI){
    throw new Error("True transparent WebM export is not supported reliably by Safari/iPhone. Use Chrome, Edge or Firefox desktop for alpha WebM.");
  }
  if(typeof el.canvas.captureStream!=="function"||typeof MediaRecorder==="undefined"){
    throw new Error("This browser cannot build a local transparent video. Use current Chrome, Edge or Firefox desktop.");
  }

  // Keep alpha export conservative for browser memory/CPU.
  const oldResolution=el.resolution.value;
  if(IS_MOBILE)el.resolution.value="720";
  configureCanvases();
  await seekTo(0);resetMasks();await renderCurrentFrame(true);

  const w=el.canvas.width,h=el.canvas.height;
  const matteCanvas=document.createElement("canvas");
  matteCanvas.width=w;matteCanvas.height=h;
  canvasToMatte(state.maskCanvas,w,h,matteCanvas);

  const fps=24;
  const colorStream=el.canvas.captureStream(fps);
  const matteStream=matteCanvas.captureStream(fps);
  const audioTracks=await prepareAudioTrack();
  const colorCombined=new MediaStream([...colorStream.getVideoTracks(),...audioTracks]);

  const colorMime=["video/webm;codecs=vp8,opus","video/webm"].find(x=>MediaRecorder.isTypeSupported(x))||"video/webm";
  const matteMime=["video/webm;codecs=vp8","video/webm"].find(x=>MediaRecorder.isTypeSupported(x))||"video/webm";
  const colorRec=new MediaRecorder(colorCombined,{mimeType:colorMime,videoBitsPerSecond:recorderBitrate(w,h)});
  const matteRec=new MediaRecorder(matteStream,{mimeType:matteMime,videoBitsPerSecond:Math.max(1800000,Math.round(recorderBitrate(w,h)*.35))});
  const colorChunks=[],matteChunks=[];
  colorRec.ondataavailable=e=>{if(e.data?.size)colorChunks.push(e.data);};
  matteRec.ondataavailable=e=>{if(e.data?.size)matteChunks.push(e.data);};
  const colorStopped=new Promise((res,rej)=>{colorRec.onstop=res;colorRec.onerror=e=>rej(e.error||new Error("Color recorder failed."));});
  const matteStopped=new Promise((res,rej)=>{matteRec.onstop=res;matteRec.onerror=e=>rej(e.error||new Error("Alpha matte recorder failed."));});

  colorRec.start(1000);matteRec.start(1000);
  setProgress(5,"Rendering transparent frames…","Recording subject color + alpha matte locally.");
  await el.video.play();
  ...
  await new Promise((resolve,reject)=>{
    ...
    const loop=async()=>{
      ...
      try{
        ...
        canvasToMatte(state.maskCanvas,w,h,matteCanvas);
        const p=5+62*(el.video.currentTime/Math.max(.1,el.video.duration));
        setProgress(p,"Rendering transparent frames…",`${formatTime(el.video.currentTime)} / ${formatTime(el.video.duration)} · ${w}×${h}`);
        if("requestVideoFrameCallback" in HTMLVideoElement.prototype)el.video.requestVideoFrameCallback(()=>loop());
        else requestAnimationFrame(()=>loop());
  ...
  canvasToMatte(state.maskCanvas,w,h,matteCanvas);
  if(colorRec.state!=="inactive")colorRec.stop();
  if(matteRec.state!=="inactive")matteRec.stop();
  await Promise.all([colorStopped,matteStopped]);

  colorCombined.getTracks().forEach(t=>t.stop());
  matteStream.getTracks().forEach(t=>t.stop());
  if(state.monitorGain)state.monitorGain.gain.value=1;

  const colorBlob=new Blob(colorChunks,{type:colorRec.mimeType||"video/webm"});
  const matteBlob=new Blob(matteChunks,{type:matteRec.mimeType||"video/webm"});
  if(!colorBlob.size||!matteBlob.size)throw new Error("Transparent pre-encode recording was empty.");

  setProgress(70,"Loading alpha encoder…","Downloading the local WebM alpha encoder only for this export.");
  const result=await mergeAlphaWebM(colorBlob,matteBlob);
...
async function exportVideo(){
  ...
  if(state.bg==="transparent"){
    ...
    try{
      ...
      el.resultMeta.textContent=`${el.canvas.width} × ${el.canvas.height} · ${(blob.size/1024/1024).toFixed(1)} MB · VP8 WebM + alpha`;
      el.result.classList.remove("hidden");
      setProgress(100,"Transparent export complete","Real alpha WebM ready · no gradient background was added.");
...
window.addEventListener("beforeunload",()=>{
  ...
  if(state.alphaWorkerBlobUrl)URL.revokeObjectURL(state.alphaWorkerBlobUrl);
});
