...
const RIVANI_VIDEO_BUILD="V45.1-CATEGORY-MASK";
queueMicrotask(()=>{
  if(el.engineNote) el.engineNote.textContent="V45.1 JS ACTIVE · category-mask engine loaded · upload a video to begin.";
  console.info("RIVANI Video Studio",RIVANI_VIDEO_BUILD,"binary category mask: 0=background, 1=person");
});
...
async function initSegmenter(){
  ...
  el.engineNote.textContent="V45.1 · Binary person category locked · 0 background / 1 person.";
...
async function segmentCurrent(force=false){
  ...
  if(!force && now-state.lastSegmentAt<(IS_MOBILE?52:34))return false;
  ...
  const result=await new Promise((resolve,reject)=>{
    let settled=false;
    try{
      const maybe=state.personSegmenter.segmentForVideo(state.inferCanvas,performance.now(),r=>{settled=true;resolve(r);});
      if(maybe&&typeof maybe.then==="function")maybe.then(r=>{if(!settled&&r)resolve(r);}).catch(reject);
      else if(maybe&&!settled&&maybe.categoryMask)resolve(maybe);
    }catch(e){reject(e);}
  });
  const categoryMask=result?.categoryMask||null;
  if(!categoryMask)throw new Error("Person category mask was not returned.");
  const mw=Math.max(1,Number(categoryMask.width)||iw), mh=Math.max(1,Number(categoryMask.height)||ih), n=mw*mh;
  if(state.maskCanvas.width!==mw||state.maskCanvas.height!==mh){state.maskCanvas.width=mw;state.maskCanvas.height=mh;state.prevPerson=null;}
  let cats;
  try{cats=categoryMask.getAsUint8Array();}
  catch(_e){const raw=categoryMask.getAsFloat32Array();cats=new Uint8Array(raw.length);for(let i=0;i<raw.length;i++)cats[i]=Number(raw[i])>=.5?1:0;}
  if(!cats||cats.length<n)throw new Error("Category mask geometry mismatch.");
  const first=!state.prevPerson||state.prevPerson.length!==n;
  if(first)state.prevPerson=new Float32Array(n);
  const c=currentControls(), keep=Math.min(.42,c.temporal*.62), personImage=new ImageData(mw,mh);
  let personPixels=0,borderPerson=0,borderN=0;
  for(let i=0,j=0;i<n;i++,j+=4){
    const target=(cats[i]|0)>0?1:0;
    const prev=first?target:state.prevPerson[i], motion=Math.abs(target-prev), adaptiveKeep=first?0:keep*(1-clamp(motion*3.2,0,.96));
    const temporal=first?target:(prev*adaptiveKeep+target*(1-adaptiveKeep)); state.prevPerson[i]=temporal;
    const threshold=.34+(c.edge/100)*.22, softness=.12;
    let p=smoothstep(threshold-softness,threshold+softness,temporal);
    if(target===1)p=Math.max(p,.985);
    if(target===0&&temporal<.34)p=0;
    const a=Math.round(255*clamp(p,0,1));
    personImage.data[j]=255;personImage.data[j+1]=255;personImage.data[j+2]=255;personImage.data[j+3]=a;
    if(a>90)personPixels++;
    const x=i%mw,y=(i/mw)|0;
    if(x<mw*.06||x>mw*.94||y<mh*.06||y>mh*.94){if(target===1)borderPerson++;borderN++;}
  }
  const coverage=personPixels/Math.max(1,n), borderRatio=borderPerson/Math.max(1,borderN);
  if(coverage>.88&&borderRatio>.72){categoryMask.close?.();throw new Error(`Foreground safety stopped an impossible mask (coverage ${Math.round(coverage*100)}%, border ${Math.round(borderRatio*100)}%).`);}
  state.maskCtx.clearRect(0,0,mw,mh);state.maskCtx.putImageData(personImage,0,0);state.lastMaskCoverage=coverage;
  if(state.faceDetector && (force || now-state.lastFaceAt>140)){
    state.lastFaceAt=now;
    try{const faceResult=state.faceDetector.detectForVideo(state.inferCanvas,performance.now());const detection=faceResult?.detections?.[0];const b=detection?.boundingBox;
      if(b){const sx=el.canvas.width/iw, sy=el.canvas.height/ih;const next={x:(b.originX-b.width*.16)*sx,y:(b.originY-b.height*.25)*sy,w:b.width*1.32*sx,h:b.height*1.48*sy};
        if(state.faceBox){const k=.58;state.faceBox={x:state.faceBox.x*k+next.x*(1-k),y:state.faceBox.y*k+next.y*(1-k),w:state.faceBox.w*k+next.w*(1-k),h:state.faceBox.h*k+next.h*(1-k)};}else state.faceBox=next;}
    }catch(faceError){console.warn("Face tracking frame skipped",faceError);}
  }
  const fctx=state.faceMaskCtx,w=state.faceMaskCanvas.width,h=state.faceMaskCanvas.height;fctx.clearRect(0,0,w,h);
  if(state.faceBox){const b=state.faceBox;fctx.save();fctx.filter="blur(8px)";fctx.fillStyle="#fff";fctx.beginPath();fctx.ellipse(clamp(b.x+b.w*.5,0,w),clamp(b.y+b.h*.5,0,h),Math.max(8,b.w*.47),Math.max(8,b.h*.50),0,0,Math.PI*2);fctx.fill();fctx.restore();}
  el.personStatus.textContent=coverage>.01?`Person ${Math.round(coverage*100)}%`:"Searching";
  el.faceStatus.textContent=state.faceBox?"Face tracked":"Person mask only";
  el.stabilityStatus.textContent="Category lock · 0 BG / 1 Person";
  categoryMask.close?.();
  return true;
}
...
function resetControls(){
  el.edgeClean.value=48;el.feather.value=1.2;el.temporal.value=28;el.light.value=18;el.smooth.value=18;el.glow.value=12;el.clarity.value=10;el.warmth.value=6;
