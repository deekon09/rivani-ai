...
queueMicrotask(()=>{if(el.engineNote)el.engineNote.textContent="V45.2 ENGINE ACTIVE · stable multiclass cutout loaded";console.info("RIVANI Video Studio",RIVANI_VIDEO_BUILD);});
...
el.clear?.addEventListener("click",()=>{
  ...
  el.preview.disabled=true;el.exportBtn.disabled=true;
  el.engineStatus.textContent="Not loaded";el.personStatus.textContent="Waiting";el.faceStatus.textContent="Waiting";el.stabilityStatus.textContent="Waiting";
  el.engineNote.textContent="V45.2 ENGINE ACTIVE · stable multiclass cutout loaded";
});
...
async function segmentCurrent(force=false){
  ...
  const baseKeep=Math.min(.36,c.temporal*.52); // much less trail than old 72%
  ...
  for(let i=0,j=0;i<n;i++,j+=4){
    ...
    const target=bgData?clamp(1-Number(bgData[i]||0),0,1):(cls===0?0:1);
    ...
    const keep=dropping?baseKeep*.20:baseKeep;
    ...
    // Human-core protection: tighten background without deleting person body.
    if(target>.72)p=Math.max(p,.985);
    else if(target>.56)p=Math.max(p,.90);
    else if(target>.42)p=Math.max(p,.66);
    if(target<.12)p=0;
  ...
  el.stabilityStatus.textContent="Stable V44.1 mask · strict cleanup";
...
async function exportVideo(){
  ...
  if(state.bg==="transparent"){
    ...
    el.status.textContent="Export uses Studio background because browser alpha-video export is unreliable.";
...
function resetControls(){
  el.edgeClean.value=58;el.feather.value=.7;el.temporal.value=24;el.light.value=18;el.smooth.value=18;el.glow.value=12;el.clarity.value=10;el.warmth.value=6;
